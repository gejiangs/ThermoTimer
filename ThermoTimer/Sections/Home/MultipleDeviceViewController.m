...
-(void)initUI
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshTableViewCell) name:NotificationBlueConnected object:nil];
}
...
-(void)replaceFoodInfo:(FoodModel *)model
{
    ...
    CGFloat temp = foodModel.gCustomValue;
...
-(void)clearCustomValueWithTag:(NSInteger)tag
{
    ...
    for (FoodModel *model in self.foods) {
        if (tag == model.tag) {
            ...
            deviceNumber = model.number;
        }
    }
    
    //同时发送锁铃命令
    if (deviceNumber > 0) {
        [[BluetoothManager shareManager] blockingWithIndex:deviceNumber];
    }
}
...
//    }
    ...
    [[BluetoothManager shareManager] deblockingWithIndex:index];
    NSArray *number = @[@"", @"①", @"②", @"③", @"④"];
    self.tempAlert = [[TKAlertView alloc] initWithTitle:[NSString stringWithFormat:[self languageKey:@"TempIndexAlarm"], [number objectAtIndex:index]]
                                                buttonTitle:[self languageKey:@"Fine"]
                                                 sureAction:^{
                                                     ...
                                                     [[BluetoothManager shareManager] blockingWithIndex:index];
